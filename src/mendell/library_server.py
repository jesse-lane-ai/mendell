...
function openClipEditor(trackIndex, startBar) {
  ...
  // Audio-type guard (audio editor lives in openAudioClipEditor; MIDI branch TBD).
  const _t = _arrTrack(trackIndex);
  const _p = _t && (_t.placements || []).find(x => x.start_bar === startBar);
  if (_p && (_p.clip_type || _t.type) === "audio") { openAudioClipEditor(trackIndex, startBar); return; }
}
